...
static AppConfig default_cfg(){
  AppConfig c; c.version=1; c.sampling_interval_s=5; c.upload_interval_s=15; c.register_mask=0x03; c.max_buffer_size=256; return c;
}
...
AppConfig config_get_active(){ return active_cfg; }

bool config_stage_from_json(const String& json, String& accepted_json, String& rejected_json){
  StaticJsonDocument<512> doc;
  if (deserializeJson(doc, json)) { rejected_json = "{\"rejected\":[\"invalid_json\"]}"; return false; }
  if (!doc.containsKey("config_update")) { rejected_json = "{\"rejected\":[\"no_config_update\"]}"; return false; }
  JsonObject u = doc["config_update"];
  staged_cfg = active_cfg;
  bool changed=false;
  if (u.containsKey("sampling_interval")) { staged_cfg.sampling_interval_s = u["sampling_interval"].as<uint32_t>(); changed=true; }
  if (u.containsKey("upload_interval")) { staged_cfg.upload_interval_s = u["upload_interval"].as<uint32_t>(); changed=true; }
  if (u.containsKey("max_buffer_size")) { staged_cfg.max_buffer_size = u["max_buffer_size"].as<uint16_t>(); changed=true; }
  if (u.containsKey("registers")) {
    ...
    staged_cfg.register_mask = mask; changed=true;
  }
  if (!changed) { rejected_json = "{\"rejected\":[\"no_changes\"]}"; return false; }
  if (!validate(staged_cfg)) { rejected_json = "{\"rejected\":[\"invalid_values\"]}"; return false; }
...
String config_build_ack_json(){
  String s = "{\"config_ack\":{\"accepted\":[\"sampling_interval\",\"upload_interval\",\"registers\"],\"rejected\":[],\"unchanged\":[]}}";
  return s;
}
